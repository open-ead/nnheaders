...
public:
    ...
    static_assert(ModuleBits + DescriptionBits + ReservedBits == sizeof(BaseType) * CHAR_BIT,
...
static_assert(std::is_trivially_destructible<Result>::value,
              "std::is_trivially_destructible<Result>::value");
...
inline Result ConstructResult(ResultTraits::BaseType v) noexcept {
    return ResultInternalAccessor::ConstructResult(v);
}
...
public:
    ...
    static constexpr typename Base::BaseType InnerValue =
        ResultTraits::MakeInnerValueStatic<Module, Description>::value;
...
public:
    static bool Includes(Result result) noexcept {
        return result.GetModule() == Module && DescriptionStart <= result.GetDescription() &&
               result.GetDescription() < DescriptionEnd;
    }

    friend bool operator<=(Result result, ErrorRange) noexcept { return Includes(result); }
};
...
#define R_DEFINE_NAMESPACE_RESULT_MODULE(value)                                                    \
    ...
    using ResultModuleId = std::integral_constant<int, value>;                                     \
    }
...
#define R_DEFINE_ERROR_RESULT_IMPL(name, desc_start, desc_end)                                     \
    class Result##name                                                                             \
        : public ::nn::result::detail::ErrorResultBase<R_CURRENT_NAMESPACE_RESULT_MODULE,          \
                                                       desc_start>,                                \
          public ::nn::result::detail::ErrorRange<R_CURRENT_NAMESPACE_RESULT_MODULE, desc_start,   \
                                                  desc_end + 1> {}
...
#define R_DEFINE_ABSTRACT_ERROR_RESULT(name, desc)                                                 \
    R_DEFINE_ABSTRACT_ERROR_RESULT_IMPL(name, desc, desc)
#define R_DEFINE_ABSTRACT_ERROR_RANGE(name, start, end)                                            \
    R_DEFINE_ABSTRACT_ERROR_RESULT_IMPL(name, start, end)
