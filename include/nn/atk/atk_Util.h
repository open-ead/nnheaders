...
public:
    ...
    static u8 DivideBy8bit(u32 value, int index) { return static_cast<u8>(value >> (8 * index)); }
    ...
    static const void* GetWaveFileOfWaveSound(const void* wsdFile, u32 index,
                                              const SoundArchive& arc,
                                              const SoundArchiveLoader& mgr);

    static ItemType GetItemType(u32 id) { return static_cast<ItemType>(id >> 24); }

    static u32 GetItemIndex(u32 id) { return id & 0x00FFFFFF; }

    static u32 GetMaskedItemId(u32 id, ItemType type) { return id | (type << 24); }
