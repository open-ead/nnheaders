...
public:
    ...
    s32 GetLastSocketError(
        long);  // unknown if this returns anything, it branches to an external function probably
                // located in nnsdk, but uses of it seem to require some sort of int
    s32 RecvFrom(uchar*, ulong, nn::nex::SocketDriver::InetAddress*, unsigned long*,
