...
bool StreamSoundPrefetchFileReader::IsValidFileHeader(const void* streamSoundPrefetchFile) const {
    const BinaryFileHeader& header{
        *util::ConstBytePtr(streamSoundPrefetchFile).Get<BinaryFileHeader>()};
...
void StreamSoundPrefetchFileReader::Initialize(const void* streamSoundPrefetchFile) {
    if (IsValidFileHeader(streamSoundPrefetchFile)) {
        m_pHeader =
            util::ConstBytePtr(streamSoundPrefetchFile).Get<StreamSoundPrefetchFile::FileHeader>();
...
bool StreamSoundPrefetchFileReader::ReadDspAdpcmChannelInfo(DspAdpcmParam* pParam,
    ...
    auto* src{m_pInfoBlockBody->GetChannelInfoTable()
...
bool StreamSoundPrefetchFileReader::ReadPrefetchDataInfo(PrefetchDataInfo* pDataInfo,
                                                         int prefetchIndex) const {
...
bool StreamSoundPrefetchFileReader::ReadRegionInfo(StreamSoundFile::RegionInfo* pInfo,
                                                   u32 regionIndex) const {  // 179
